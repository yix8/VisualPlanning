...
def action_reward_func(prompts, completions, action_parser, display, num_generations, **kwargs) -> list[float]:
    ...
    for i in range(len(prompts)):
        ...
        action_info = action_parser.parse_action_in_ids(
            prompts[i:i+1], completions[i:i+1], level, kwargs['input_state'][i], meta['target_pos'], display
        )
...
def main(cfg: DictConfig):
    ...
    merged_model.save_pretrained(f"./models/{cfg.GRPO.run_name}_merged_ckpts")
